...
class HorizontalSlice:
    def __init__(self, name, height, frequency, var, state, Sim):
        ...
        self._out_path = os.path.join(
            self._Sim._namelist["meta"]["output_directory"],
            self._Sim._namelist["meta"]["simname"],
        )

        self._out_path = os.path.join(
            self._out_path, "HorizontalSlice_" + str(self._height) + "_" + var
        )
    ...
    def _compute_height_index(self):
        ...
        self._level = np.where(
            np.abs(zl - self._height) == np.amin(np.abs(zl - self._height))
        )[0][0]
    ...
    def update(self):
        ...
        local_array[
            local_start[0] : local_end[0], local_start[1] : local_end[1]
        ] = state[n_halo[0] : -n_halo[0], n_halo[1] : -n_halo[1]]
        ...
        fname = os.path.join(
            self._out_path, str(100000000 + np.round(time, 1)) + ".pkl"
        )
...
class Albedo:
    ...
    def update(self):
        ...
        tau = np.sum(
            ...
            * Nc ** (1.0 / 3.0)
            * (
                (state * 1000.0 * rho[np.newaxis, np.newaxis, :] * dx[0]) ** (5.0 / 6.0)
            ),
            axis=2,
        )
        tau = tau / (6.8 + tau)

        local_array[local_start[0] : local_end[0], local_start[1] : local_end[1]] = tau[
