            ...
            for j in range(1, shape[1] - 1):
                for k in range(1, shape[1] - 1):
                    ...
                    _ut = (
                        0.2
                        * 0.5
                        * (LAI[i, j, k] + LAI[i - 1, j, k])
                        * np.sqrt(_u ** 2.0)
                        * _u
                    )
                    ut[i, j, k] -= np.sign(_u) * min(
                        np.abs(_ut * dt), np.abs(0.8 * _u / dt)
                    )
                    _vt = (
                        0.2
                        * 0.5
                        * (LAI[i, j, k] + LAI[i, j - 1, k])
                        * np.sqrt(_v ** 2.0)
                        * _v
                    )
                    vt[i, j, k] -= np.sign(_v) * min(
                        np.abs(_vt * dt), np.abs(0.8 * _v / dt)
                    )
                    _wt = (
                        0.2
                        * 0.5
                        * (LAI[i, j, k] + LAI[i, j, k - 1])
                        * np.sqrt(w[i, j, k] ** 2.0)
                        * w[i, j, k]
                    )
                    wt[i, j, k] -= np.sign(w[i, j, k]) * min(
                        np.abs(_wt * dt), np.abs(0.8 * w[i, j, k] / dt)
                    )
