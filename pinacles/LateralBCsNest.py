...
class LateralBCsNest(LateralBCsBase):
    ...
    def set_vars_on_boundary(self, **kwargs):
        ...
        for var_name in self._State._dofs:
            ...
            #import sys; sys.exit()
            y_low[nh[0] : -nh[0], nh[2] : -nh[2]]  = np.repeat(self._NestState.get_slab_y(var_name, slab_range)[
                local_part_of_parent[0] : local_part_of_parent[1], :, :
            ], self.factor[1], axis=0)[:self._Grid._local_shape[0],0,:]
            ...
            y_high[nh[0] : -nh[0], nh[2] : -nh[2]] =  np.repeat(self._NestState.get_slab_y(var_name, slab_range)[
                local_part_of_parent[0] : local_part_of_parent[1], :, :
            ], self.factor[1],axis=0)[:self._Grid._local_shape[0],0,:]
            ...
            x_low[nh[1] : -nh[1], nh[2] : -nh[2]] = np.repeat(self._NestState.get_slab_x(var_name, slab_range)[
                :, local_part_of_parent[0] : local_part_of_parent[1], :
            ], self.factor[0],axis=1)[0,:self._Grid._local_shape[1],:]
            ...
            x_high[nh[1] : -nh[1], nh[2] : -nh[2]] = np.repeat(self._NestState.get_slab_x(var_name, slab_range)[
                :, local_part_of_parent[0] : local_part_of_parent[1], :
            ], self.factor[0], axis=1)[0,:self._Grid._local_shape[1],:]
