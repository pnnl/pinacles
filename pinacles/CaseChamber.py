...
def initialize(namelist, ModelGrid, Ref, ScalarState, VelocityState):
    ...
    pert_mean = np.mean(
        np.mean(pert[nh[0] : -nh[0], nh[1] : -nh[1], :], axis=0), axis=0
    )
...
class ForcingChamber(Forcing.ForcingBase):
    ...
    def update(self):
        ...
        for noz_point in noz_points:
            ...
            if (
                on_rank
                and self._TimeSteppingController.time >= 1.0
                and self._TimeSteppingController.time <= 250.0
            ):
        ...
        Forcing_impl.large_scale_pgf(
            self._ug, self._vg, self._f, u, v, 0.0, 0.0, vt, ut
        )
