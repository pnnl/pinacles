...
class JaxPrognostic(Pytree, mutable=True):
    ...
    def set_tend(self, var, data):
        return self.replace(tend_array=self.tend_array.at[self.dofs[var], :, :, :].set(data))

    @partial(
    jax.jit,static_argnums=(0, 1), inline=True)
...
class JaxStep:
    ...
    def jax_update(
        ...
        Velocities, Diagnostics = ThermodynamicsDry.update_jax(Grid, Ref, Scalars, Velocities, Diagnostics)
        Diagnostics = JaxKinematic.update_jax(Grid, Ref, Scalars, Velocities, Diagnostics)
        Diagnostics = JaxSmagorinsky.update_jax(Grid, Ref, Scalars, Velocities, Diagnostics)
        Scalars = JaxScalarDiffusion.update_jax(Grid, Ref, Scalars, Velocities, Diagnostics)
