...
class Nest:
    ...
    @staticmethod
    @numba.njit()
    def relax_x_parallel(n_halo, factor,  indx_range, parent_var, var, var_tend):
        ...
        for i in range(x_indx_range[0],  x_indx_range[1]):
            ...
            for j in range(y_indx_range[0], y_indx_range[1]):
                ...
                for k in range(z_indx_range[0],  z_indx_range[1]):
                    ...
                    var_tend[i,j,k] += 1.0/10.0 * (parent_var[i_parent,j_parent,k_parent] - var[i,j,k])
    ...
    @staticmethod
    @numba.njit()
    def relax_y_parallel(n_halo, factor,  indx_range, parent_var, var, var_tend):
        ...
        y_indx_range =  (2*n_halo[1], var_shape[1]-2*n_halo[1])#(2*n_halo[1], var_shape[1] - 2*n_halo[1]) #(n_halo[1] + factor, var_shape[1] - n_halo[1]-factor) #(indx_range[0], indx_range[0] + (indx_range[1]-indx_range[0])*factor)
        z_indx_range = (n_halo[2], var_shape[2] - n_halo[2])   #Loop over the full range of z w/o halos
    ...
    def update_boundaries(self, ParentNest):
        ...
        for v in self._ScalarState._dofs:
            ...
            #print(v,  np.amax(self.x_left_bdys[v]))
            #import sys;  sys.exit()

            self.x_right_bdys[v] = ParentNest.ScalarState.get_slab_y(v,
                                                                (center_point_y + self.partent_pts-1, center_point_y+self.partent_pts)
                                                                 )[local_part_of_parent[0]:local_part_of_parent[1],:,:]
            ...
            self.y_left_bdys[v] = ParentNest.ScalarState.get_slab_x(v, (center_point_x, center_point_x+1))[:,local_start[1]//self.factor:local_end[1]//self.factor,:]
            self.y_right_bdys[v] = ParentNest.ScalarState.get_slab_x(v, (center_point_x + self.partent_pts-1, center_point_x+self.partent_pts))[:,local_start[1]//self.factor:local_end[1]//self.factor,:]

            #print(np.amax(self.y_right_bdys[v]))
            #print(np.amin(self.y_right_bdys[v]))
            #print(self.y_left_bdys[v])
        ...
        #Now get the indicies of the subset on this rank
        local_part_of_parent =  (local_start[0]//self.factor,local_end[0]//self.factor)
        ...
        self.x_right_bdys[v] = ParentNest.VelocityState.get_slab_y(v,
                                                            (center_point_y + self.partent_pts, center_point_y+1+self.partent_pts)
                                                                )[local_part_of_parent[0]:local_part_of_parent[1],:,:]

        
        #print(ParentNest.ModelGrid.y_local[center_point_y], ParentNest.ModelGrid.y_local[center_point_y + self.partent_pts])
        #import sys; sys.exit()

        #print(ParentNest.ModelGrid.x_local[center_point_x], ParentNest.ModelGrid.y_local[center_point_y])
        #print(ParentNest.ModelGrid.x_local[local_start[0]//self.factor], ParentNest.ModelGrid.x_local[local_end[0]//self.factor])
        #import sys; sys.exit()


        self.y_left_bdys[v] = ParentNest.VelocityState.get_slab_x(v, (center_point_x, center_point_x+1))[:,local_start[1]//self.factor:local_end[1]//self.factor,:]
        self.y_right_bdys[v] = ParentNest.VelocityState.get_slab_x(v, (center_point_x + self.partent_pts, center_point_x+1+self.partent_pts))[:,local_start[1]//self.factor:local_end[1]//self.factor,:]
       ...
       #import sys; sys.exit()
        ...
        #for v in ['u', 'v', 'w']:
        #    
        #    center_point_x = parent_nhalo[0] + self.root_point[0] - 1
        #    center_point_y = parent_nhalo[1] + self.root_point[1] - 1
        #    
        #    self.x_left_bdys[v] = self._VelocityState.get_slab_y(v, (center_point_y, center_point_y+1))
        #    self.x_right_bdys[v] = self._VelocityState.get_slab_y(v, (center_point_y + self.partent_pts, center_point_y+1+self.partent_pts))
        #   
        #    self.y_left_bdys[v] = self._VelocityState.get_slab_x(v, (center_point_x, center_point_x+1))
        #    self.y_right_bdys[v] = self._VelocityState.get_slab_x(v, (center_point_x + self.partent_pts, center_point_x+1+self.partent_pts))
    ...
    def update(self, ParentNest):
        ...
        for v in ['s']:
           ...
           # print('Left Bdy')
            self.relax_x_parallel(n_halo, self.factor,  indx_range, self.x_left_bdys[v], var, var_tend)

            #print('Right Bdy')
            #indx_range = (n_halo[1]-1 + self._Grid.local_shape[1]-1, n_halo[1]-1 + self._Grid.local_shape[1])
            indx_range = (var.shape[1] - 2 * n_halo[1],var.shape[1] - n_halo[1])
            self.relax_x_parallel(n_halo, self.factor,  indx_range, self.x_right_bdys[v], var, var_tend)
            #print(indx_range)

            indx_range = (n_halo[0], n_halo[0]+1)
            self.relax_y_parallel(n_halo, self.factor,  indx_range, self.y_left_bdys[v], var, var_tend)  

            indx_range = (var.shape[0] - 2 * n_halo[0],var.shape[1] - n_halo[0])
            self.relax_y_parallel(n_halo, self.factor,  indx_range, self.y_right_bdys[v], var, var_tend) 
        ...
        self.relax_y_parallel(n_halo, self.factor,  indx_range, self.y_right_bdys[v], var, var_tend) 
            ...
            #if v == 's':
            #    import pylab as plt
            #    plt.contourf((self.x_left_bdys[v][:,0,:]))
            #    plt.show()


            #print(np.amax(self.x_left_bdys[v]),  np.amin(self.x_left_bdys[v]))
            #import sys; sys.exit()

            #self.relax_parallel(root_point, partent_pts, parent_nhalo, factor, tau_i, parent_scalar, n_halo, scalar, scalar_tend)
