...
class Nest:
    ...
    def relax_x_parallel(n_halo, factor,  indx_range, tau_i, parent_var, var, var_tend):
        ...
        y_indx_range = (indx_range[0], indx_range[1])# + (indx_range[1]-indx_range[0])*factor[1])
    ...
    @staticmethod
    @numba.njit()
    def relax_y_parallel(n_halo, ls, le, n, factor,  indx_range, tau_i, parent_var, var, var_tend):
        ...
        x_indx_range = (indx_range[0], indx_range[1]) #(n_halo[0], var_shape[0] - n_halo[0])   #Loop over the full range of x w/o halos
        ...
        for i in range(x_indx_range[0],  x_indx_range[1]):
            i_parent = (i- x_indx_range[0])//factor[0] 
            for j in range(y_indx_range[0], y_indx_range[1]):
                if (j >= n_halo[1] + factor[1] and j  < var_shape[1]- n_halo[1] - factor[1]):
                    j_parent = (j-y_indx_range[0]) //factor[1]
                    for k in range(z_indx_range[0],  z_indx_range[1]):
                        k_parent = (k - z_indx_range[0]) //factor[2]
    ...
    def update(self, ParentNest):
        ...
        nest_y_left_bdy = np.amin(self._Grid._global_axes_edge[1]) ==  np.amin(self._Grid._local_axes_edge[1])
        nest_y_right_bdy = np.max(self._Grid._global_axes_edge[1]) ==  np.max(self._Grid._local_axes_edge[1])
