...
class RRTMG:
    def __init__(
        ...
        try:
            ...
            if namelist["meta"]["casename"] == "testbed":
                ...
                UtilitiesParallel.print_root(
                    "\t \t Assuming RRTMG should be used for testbed case."
                )
            else:
        ...
        ffi.cdef(
            "void c_rrtmg_sw(int ncol, int nlay, int icld, int iaer, double play[], \
        ...
        if len(self._latitude) > 1:
            try:
                ...
                UtilitiesParallel.print_root(
                    " WARNING: No radiation data times given, using forcing times!"
                )
    ...
    def init_profiles(self):
        ...
        lw_absorber = np.where(
            lw_absorber > 2.0, np.zeros_like(lw_absorber), lw_absorber
        )
        index_o3 = 7

        self._profile_o3 = interpolate_trace_gas(
    ...
    def update(self, force=False):
        ...
        if (
            ...
            _nhalo = self._Grid.n_halo
            _ncol = (_ngrid_local[0] - 2 * _nhalo[0]) * (
                _ngrid_local[1] - 2 * _nhalo[1]
            )
            _nlay = (
                _ngrid_local[2]
                - 2 * _nhalo[2]
            ...
            if len(np.shape(self._Surf.T_surface)) == 0:
                ...
                tsfc = np.ravel(self._Surf.T_surface[3:-3, 3:-3])
            ...
            if self._data_times is None:
                ...
                coszen_cur = cos_sza(self.dyofyr, self.hourz, lat_cur, lon_cur)
            ...
            tlev = interpolate.interp1d(
                p_temp[0, :], t_temp, axis=1, fill_value="extrapolate"
            )(plev[0, :])
            ...
            cliqwp[:, :] = (
                cliqwp[:, :] * 1.0e3 / parameters.G * (plev[:, :-1] - plev[:, 1:])
            )
            ...
            self._lib_lw.c_rrtmg_lw(
                ...
                liqflglw,
                as_pointer(cldfr),
                as_pointer(taucld_lw),
                as_pointer(cicewp),
                as_pointer(cliqwp),
                as_pointer(reice),
                as_pointer(reliq),
                as_pointer(tauaer_lw),
                as_pointer(uflx_lw),
                as_pointer(dflx_lw),
                as_pointer(hr_lw),
                as_pointer(uflxc_lw),
                ...
                as_pointer(hrc_lw),
                as_pointer(duflx_dt),
                as_pointer(duflxc_dt),
            )

            self._lib_sw.c_rrtmg_sw(
                _ncol,
                _nlay,
                icld,
                iaer,
                ...
                liqflgsw,
                as_pointer(cldfr),
                as_pointer(taucld_sw),
                as_pointer(ssacld_sw),
                as_pointer(asmcld_sw),
                as_pointer(fsfcld_sw),
                as_pointer(cicewp),
                as_pointer(cliqwp),
                as_pointer(reice),
                as_pointer(reliq),
                as_pointer(tauaer_sw),
                as_pointer(ssaaer_sw),
                as_pointer(asmaer_sw),
                as_pointer(ecaer_sw),
                as_pointer(uflx_sw),
                as_pointer(dflx_sw),
                as_pointer(hr_sw),
...
def cos_sza(jday, hourz, dlat, dlon):
    ...
    day_angle = 2.0 * np.pi * (jday - 1.0) / dpy
    # Hours of Meridian Passage (true solar noon)
    homp = (
        12.0
        + 0.12357 * np.sin(day_angle)
        - 0.004289 * np.cos(day_angle)
        + 0.153809 * np.sin(2 * day_angle)
        + 0.060783 * np.cos(2 * day_angle)
    )
    hour_angle = dph * (hourz - homp) - dlon
    ang = 279.9348 * np.pi / 180.0 + day_angle
    sigma = (
        (
            ang * 180.0 / np.pi
            + 0.4087 * np.sin(ang)
            + 1.8724 * np.cos(ang)
            - 0.0182 * np.sin(2.0 * ang)
            + 0.0083 * np.cos(2.0 * ang)
        )
        * np.pi
        / 180.0
    )
    sindlt = sinob * np.sin(sigma)
    cosdlt = np.sqrt(1.0 - sindlt * sindlt)
    return np.maximum(
        sindlt * np.sin(np.pi / 180.0 * dlat)
        + cosdlt * np.cos(np.pi / 180.0 * dlat) * np.cos(np.pi / 180.0 * hour_angle),
        0.0,
    )
...
def interpolate_trace_gas(p_trace_pa, trace_vmr, p_pa):
    ...
    for i in range(1, nlev):
        ...
        if p_pa[i - 1] > p_trace_pa[0]:
            trpath[i] += (
                (p_pa[i - 1] - np.maximum(p_pa[i], p_trace_pa[0])) / 9.81 * trace_vmr[0]
            )
        for m in range(1, ntrace):
            ...
            if plow > pupp:
                ...
                wgtupp = (p_trace_pa[m - 1] - pmid) / (
                    p_trace_pa[m - 1] - p_trace_pa[m]
                )
                trpath[i] += (
                    (plow - pupp)
                    / 9.81
                    * (wgtlow * trace_vmr[m - 1] + wgtupp * trace_vmr[m])
                )
        if p_pa[i] < p_trace_pa[-1]:
            trpath[i] += (
                (np.minimum(p_pa[i - 1], p_trace_pa[-1]) - p_pa[i])
                / 9.81
                * trace_vmr[-1]
            )
