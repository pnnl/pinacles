...
class ReferenceBase:
    ...
    def _boundary_update_edge(self, prof_array):
        ...
        prof_array[-n_halo:] = prof_array[-2 * n_halo - 1 : -2 * n_halo + n_halo - 1][
            ::-1
        ]
...
class ReferenceDry(ReferenceBase):
    ...
    def integrate(self):
        ...
        self._P0_edge[nhalo - 1 : -nhalo + 1] = _integrate_dry(
            self._Grid.z_edge_global[nhalo - 1 : -nhalo + 1], lnp_sfc, self.ssfc
        )

        # Compute reference temperature profiles
        self._T0[nhalo:-nhalo] = ThermodynamicsDry_impl.T(z, self.ssfc)[1:]
        self._T0_edge[nhalo - 1 : -nhalo + 1] = ThermodynamicsDry_impl.T(
            self._Grid.z_edge_global[nhalo - 1 : -nhalo + 1], self.ssfc
        )
    ...
    def write_stats(self, nc_ref_grp):
        ...
        alpha0 = nc_ref_grp.createVariable("alpha0", np.single, dimensions=("z"))
        ...
        alpha0_edge = nc_ref_grp.createVariable(
            "alpha0_edge", np.single, dimensions=("z_edge")
        )
        ...
        exner = nc_ref_grp.createVariable("exner", np.single, dimensions=("z"))
        exner.long_name = "Reference state exner function at cell center"
        exner.latex_name = "\pi_0"
        exner.units = ""
        exner[:] = self._exner[nh[2] : -nh[2]]

        exner_edge = nc_ref_grp.createVariable(
            "exner_edge", np.single, dimensions=("z_edge")
        )
