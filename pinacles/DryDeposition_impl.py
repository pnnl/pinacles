...
@numba.njit
def v_gravitational(
    particle_density, particle_diameter, temperature, air_density, pressure
):
...
def cunningham_factor(particle_diameter, temperature, air_density, pressure):
    ...
    c = 1.0 + 2.0 * mfp / particle_diameter * (
        1.257 + 0.4 * np.exp(-0.55 * particle_diameter / mfp)
    )
    return c
...
@numba.njit
def compute_dry_deposition_velocity_zhang(
    dry_particle_diameter, T, rh, nh, z, rho0, p0, shf, lhf, ustar, z0, vdep
):
    shape = T.shape
    ...
    for i in range(nh[0] - 1, shape[0] - nh[0] + 1):
        for j in range(nh[1], shape[1] - nh[1] + 1):
            ...
            bflux = (
                ...
                + (parameters.EPSVI - 1.0) * lhf[i, j] / parameters.LV
            ) / rho0[nh[2]]
            if bflux > 1e-5:
            ...
            for k in range(nh[2] - 1, shape[2] - nh[2] + 1):
                rd = dry_particle_diameter * 0.5
                particle_diameter = 2.0 * (
                    C1 * rd ** C2 / (C3 * rd ** C4 - np.log(rh[i, j, k])) + rd ** 3.0
                )
                vg = v_gravitational(
                    particle_density, particle_diameter, T[i, j, k], rho0[k], p0[k]
                )
                R_surf = r_surface(
                    particle_diameter, rho0[k], T[i, j, k], vg, ustar[i, j]
                )
...
@numba.njit
def compute_dry_deposition_velocity_gravitational(
    dry_particle_diameter, T, rh, nh, rho0, p0, vdep
):
    shape = T.shape
        ...
        for j in range(shape[1]):
            for k in range(shape[2]):
                pv = (
                    ...
                    * parameters.EPSVI
                    * qv[i, j, k]
                    / (1.0 - qv[i, j, k] + parameters.EPSVI * qv[i, j, k])
                )
                Tcel = T[i, j, k] - 273.15
    ...
    for i in range(nh[0], shape[0] - nh[0]):
        for j in range(nh[1], shape[1] - nh[1]):
            for k in range(nh[2], shape[2] - nh[2]):
                phi_t[i, j, k] += (
                    vdep[i, j, k] * (phi[i, j, k + 1] - phi[i, j, k]) * dxi_2
                )
