...
!bloss         call derm1 ('DERFI',1,2,'Argument out of range','X',x,'.')
         write(*,*) 'Argument out of range in DERFI, X = ',x
         STOP 'in DERFI'
      end if
...
cbloss         call derm1('DERFCI',1,2,'Argument out of range','X',x,'.')
         write(*,*) 'Argument out of range in DERFCI, X = ',x
         STOP 'in DERFCI'
      end if
cblossc     In case the error level is shifted to zero by the caller:
cbloss         derfci = 0.0d0
cbloss      end if
      if (x.eq.1.0d0) then
         derfci = 0.0d0
...
c
      if (first) then
         first = .false.
cbloss: Remove dependency on amach.f
cbloss         s2 = 0.5d0*d1mach(3)
         s2 = 0.5*EPSILON(1.)
         do 120 imin = -1, 5, 2
