...
! added for diagCCN
 	...
 	double precision :: gmax,tmax,qmax,divmax,rainmax,qnmax,inmax,knmax,hydro,difmax, tdif, tt_old, w_stag, w_stag_my, qq_old,teten,es
 	integer  print_int
 	...
 	double precision :: FACTZ_new(KMS:KME,NKR), TT_r
 ! ### (KS) ............................................................................................
...
!---YZ2020Mar----------------------------------------------@
        ...
        if (diagCCN) then  ! for diagCCN
           ndrop=ndrop+chem_new(I,K,J,KR)*col   
                   enddo  ! end DO kr

           FCCND(:) = fccnorig(:)
           tot0 = sum(fccnorig(:))*FACTZ*COL
           if (ndrop >= tot0)  FCCND(:) = 0.0
!
           if (ndrop >= fccnorig(:)*FACTZ*COL) then
             subtot=0.0
             do krr = nkr, 1, -1
              subtot=subtot+fccnorig(krr)*FACTZ*COL
              FCCND(krr)=0.0
      ...
      DO j = jts,jte
          DO i = its,ite
            ...
            DO k = kts,kte
              ...
              if(SUP_WATER > 0.0D0 .and. w_stag_my > 0.1*1.0D2 .and. .NOT. K_found .and. K > 2 .and. zcgs(I,K,J) < 3.0*1.0D5)then
...
! ... Snow
          ...
          do kr=p_ff5i01,p_ff5i33
            KRR=KRR+1
            FF3R_D(KRR)=(1.0/RHOCGS(I,K,J))*chem_new(I,K,J,KR)*XS(KRR)*XS(KRR)*3.0
            FF3R (KRR) = chem_new(I,K,J,KR)
...
#define DM_BCAST_MACRO_R16(A) CALL wrf_dm_bcast_bytes(A, size(A)*R16SIZE)
...
#endif
     print *, 'FAST_SBM_INIT : succesfull reading Table-5'
     !CALL wrf_debug(000, errmess)
 ! +----------------------------------------------------------------------+
 ...
 ! ... KERNELS DEPENDING ON PRESSURE :
 IF ( .TRUE.) THEN
 	print *, 'module_mp_WRFsbm : Table-7 -- opening kernels33.asc on unit',hujisbm_unit1
 	!CALL wrf_debug(150, errmess)
 ...
 ! +-----------------------------------------------------------+
     ...
     ENDIF
     IF ( .TRUE.) THEN
         print *,'module_mp_FAST_SBM : Table-9 -- opening bulkradii.asc on unit',hujisbm_unit1
         !CALL wrf_debug(150, errmess)
...
!---YZ2020Mar:read aerosol size distribution from observation----@
        ELSE ! read an observed SD with a format of aerosol size (cm), dN (#cm-3) and dNdlogD for 33bins (Jinwe Fan)
            ...
            CLOSE(hujisbm_unit1)
            print *, "FAST_SBM_INIT: succesfull reading aerosol SD from observation"
        ENDIF
!-------------------------------------------------------------@
 ! +-------------------------------------------------------------+
