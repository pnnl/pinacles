            ...
            for k in range(shape[2]):
                ...
                for p in range(3):
                    for r in range(3):
                        for s in range(3):
                            phi_new += (
                                coeff_tensor[i, j, k, p, r, s]
                                * phi[i + p - 1, j + r - 1, k + s - 1]
                            )
...
@numba.njit(fastmath=True)
def compute_SL_tend_bounded_tvd(
    u,
    v,
    w,
    phi,
...
class CTU(ScalarAdvection.ScalarAdvectionBase):
    def __init__(self, namelist, Grid, Ref, ScalarState, VelocityState, TimeStepping):
        ...
        ScalarAdvection.ScalarAdvectionBase.__init__(
            self, Grid, Ref, ScalarState, VelocityState, TimeStepping
        )
        coeff_shape = (
            self._Grid.ngrid_local[0],
            self._Grid.ngrid_local[1],
            self._Grid.ngrid_local[2],
            3,
            3,
            3,
        )
    ...
    def update(self):
        ...
        compute_coeff_tensor_biquad(
            u, v, w, dxi, dt, x_quad, y_quad, z_quad, self._coeff_tensor2
        )
