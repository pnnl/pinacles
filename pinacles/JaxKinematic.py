...
def update_jax(Grid, Ref, Scalars, Velocities, Diagnostics):
    ...
    dudy =  0.25 * (dudy[1:,1:,1:]  + dudy[:-1,1:,1:] + dudy[1:,:-1,1:] + dudy[:-1,:-1,1:])
    dudz =  0.25 * (dudz[1:,1:,1:]  + dudz[:-1,1:,1:] + dudz[1:,1:,:-1] + dudz[:-1,1:,:-1,])
    
    dvdx = 0.25 * (dvdx[1:,1:,1:]  + dvdx[:-1,1:,1:] + dvdx[1:,:-1,1:] + dvdx[:-1,:-1,1:])
    dvdz = 0.25 * (dvdz[1:,1:,1:]  + dvdz[1:,:-1,1:] + dvdz[1:,1:,:-1] + dvdz[1:,:-1,:-1,])
    
    dwdx = 0.25 * (dwdx[1:,1:,1:]  + dwdx[:-1,1:,1:] + dwdx[1:,1:,:-1] + dwdx[:-1,1:,:-1,])
    dwdy =  0.25 * (dwdy[1:,1:,1:]  + dwdy[1:,:-1,1:] + dwdy[1:,1:,:-1] + dwdy[1:,:-1,:-1,])
    ...
    qcrit = qcrit.at[2:,2:,2:].set(0.5 * (
                    jnp.sqrt(q12 + q13 + q21 + q23 + q31 + q32)
                    - jnp.sqrt(s11[1:,1:,1:] + s22[1:,1:,1:] + s33[1:,1:,1:] + s12 + s21 + s13 + s31 + s23 + s32)
                ))
    ...
    helicity = helicity.at[2:,2:,2:].set( 0.5 * (
