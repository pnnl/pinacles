...
class DiagnosticsTurbulence:
    ...
    def io_initialize(self, this_grp):
        ...
        v = profiles_grp.createVariable("thetali", np.single, dimensions=("time", "z",))
        v.long_name = "liquid-ice potential temperature"
        v.latex_name = "\theta_{li}"
        v.units = "K"

        v = profiles_grp.createVariable("qt", np.single, dimensions=("time", "z",))
        v.long_name = "total water mixing ratio"
        v.latex_name = "q_t"
        v.units = "kg kg^{-1}"

        # 2nd moments
        v = profiles_grp.createVariable("s2", np.single, dimensions=("time", "z",))
        v.long_name = "second central moment of frozen static energy"
        v.latex_name = "\overline{s^\prime s^\prime}"
        v.units = "K^2"

        v = profiles_grp.createVariable("qv2", np.single, dimensions=("time", "z",))
        ...
        v = profiles_grp.createVariable(
            "thetali4", np.single, dimensions=("time", "z",)
        )
        ...
        v = profiles_grp.createVariable("qt4", np.single, dimensions=("time", "z",))
        ...
        for var in self._ScalarState.names:
            if not "ff" in var:  # Avoid SBM Bins
                v = profiles_grp.createVariable(
                    "w" + var, np.single, dimensions=("time", "z",)
                )
                v.long_name = (
        ...
        v = profiles_grp.createVariable(
            "qtthetali", np.single, dimensions=("time", "z",)
        )
