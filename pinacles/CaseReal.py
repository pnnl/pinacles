...
class InitializeReanalysis:
    ...
    def initialize(self):
        ...
        random = np.random.uniform(-0.1, 0.1, size=(s.shape[0], s.shape[1], 3)) * 0.0
...
class LateralBCsReanalysis(LateralBCsBase):
    ...
    def lateral_nudge(self):
        ...
        weight_u = (1.0 - np.tanh(self._Grid._edge_dist_u / dx[0] / 2)) / (250.0)
        weight_u = self.nudge_half(weight_u)

        # weight_u[self.nudge_width:-self.nudge_width,self.nudge_width:-self.nudge_width]  *= 0.5

        weight_v = (1.0 - np.tanh(self._Grid._edge_dist_v / dx[1] / 2)) / (250.0)

        weight_v = self.nudge_half(weight_v)

        weight_w = (1.0 - np.tanh(self._Grid._edge_dist / dx[0] / 2)) / (250.0)
        ...
        for var in self._State._dofs:
            ...
            if var == "u":
                ...
                start = self._Grid._ibu_edge[0] - self.nudge_width - 1
                end = self._Grid.ibu_edge[0] - 1
                ...
                if False: #self._Grid.low_rank[1]:
                ...
                if False: #self._Grid.high_rank[1]:
                ...
                if False: #self._Grid.low_rank[0]:
                ...
                if False:#self._Grid.high_rank[0]:
                    vt[start:end, corner_low:corner_high, :] -= (
                        v[start:end, corner_low:corner_high, :]
                        - v_nudge[:-1, corner_low:corner_high, :]
                    ) * weight_v[-self.nudge_width :, :, np.newaxis]
                ...
                start = (
                    self._Grid._ibu_edge[1] - self.nudge_width  - 1 
                )  # -nh[1] - self.nudge_width - 1
                end = self._Grid._ibu_edge[1] - 1  # -nh[1] - 1
                ...
                wt[nh[0]:-nh[0], nh[1]:-nh[1],:] -= w[nh[0]:-nh[0], nh[1]:-nh[1],:] * weight_w[:,:,np.newaxis]
            ...
            elif False: #var == 's' or var == 'qv':  # or var == "qc" or var=="qi" or var == 'qi1':
