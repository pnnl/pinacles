    ...
    for i in range(n_nests):
        ...
        for dim in range(2):
            ...
            nest_namelist["grid"]["n"][dim] = (
                namelist["nests"]["parent_points"][i][dim]
                * namelist["nests"]["factor"][i][dim]
            )
            nest_namelist["grid"]["l"][dim] = (
                namelist["nests"]["parent_points"][i][dim]
                * parent_namelist["grid"]["l"][dim]
        ...
        try:
            nest_namelist["nest"]["two_way"] = namelist["nests"]["two_way"]
        except:
            nest_namelist["nest"]["two_way"] = False
        ...
        this_sim = SimulationStandard.SimulationStandard(
            nest_namelist, llx, lly, llz, ParentNest=parent_sim, nest_num=i + 1
        )
