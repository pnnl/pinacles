...
def main(namelist):
    ...
    for i in range(n_nests):
        ...
        nest_namelist = copy.deepcopy(
            namelist
        )  # This will be used as a namelist for the i + 1 th nest
        parent_namelist = ListOfSims[
            i
        ]._namelist  # This is the namelist of the i-th nest
        ...
        for dim in range(2):
            ...
            assert (
                namelist["nests"]["factor"][i] % 2 != 0
            )  # Check that the nest refinment factor is odd

            # Compute the number of points and domain physical size for the i+1th nest
            nest_namelist["grid"]["n"][dim] = (
                namelist["nests"]["parent_points"][i] * namelist["nests"]["factor"][i]
            )
            nest_namelist["grid"]["l"][dim] = (
                namelist["nests"]["parent_points"][i]
                * parent_namelist["grid"]["l"][i]
                / parent_namelist["grid"]["n"][dim]
            )

        nest_namelist["grid"]["n"][2] = (
            parent_namelist["grid"]["n"][2] * namelist["nests"]["factor"][2]
        )
        ...
        nest_namelist["meta"]["simname"] = (
            namelist["meta"]["simname"] + "_nest_" + str(i)
        )

        # Set up nested lateral boundary conditions
        nest_namelist["lbc"] = {}
        ...
        llx = parent_sim.ModelGrid.x_edge_global[
            parent_sim.ModelGrid.n_halo[0] - 1 + namelist["nests"]["ll_corner"][i][0]
        ]
        lly = parent_sim.ModelGrid.y_edge_global[
            parent_sim.ModelGrid.n_halo[1] - 1 + namelist["nests"]["ll_corner"][i][1]
        ]
        llz = 0.0  # This will always be zero

        # Instantiate and initialize the
        this_sim = SimulationStandard.SimulationStandard(
            nest_namelist, llx, lly, llz, nest_num=i + 1, ParentNest=ListOfSims[-1])
    ...
    while Sim.TimeSteppingController.time < Sim.TimeSteppingController.time_max:
        ...
        for n, Nest_i in enumerate(ListOfSims):
            for idx, item in enumerate(io_classes[n]):
                if (
                    time - io_frequencies[n * len(io_classes[n]) + idx]
                    == last_io_time[n * len(io_classes[n]) + idx]
                ):
                    if hasattr(item, "update"):
                        item.update()
                    elif hasattr(item, "dump_restart"):
