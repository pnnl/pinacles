...
!      if (KR_SNOW_MAX-3<=NKR) BREAK_SNOW(KR_SNOW_MAX-3) = BREAK_SNOW_KRMAX_3
   ...
   ! +-----------------------------------------------------------------------------+
    ...
    private
    public :: POLYSVP, POLYSVP_old, JERRATE_KS, JERTIMESC_KS, JERSUPSAT_KS, &
...
! + -------------------------------------------------------- +
   ...
   ! +-----------------------------------------------------------------------------+
    ...
    private
    public JERNUCL01_KS, LogNormal_modes_Aerosol_ACPC,HISCALE_aeroInput,water_nucleation, &
    ice_nucl, cloud_base_super, supmax_coeff
   ...
   ! +-----------------------------------------------------------------------------------------------------------+
      ...
      ! ... Interface
          ...
          double precision ,intent(out) :: FCCNR_bl(:),FCCNR_ft(:)
      ...
      ! ... Local
          ...
          double precision ,PARAMETER :: RCCN_MAX = 0.5D-4         ! [cm]
          double precision ,PARAMETER :: RCCN_MIN = 0.005D-4		    ! [cm]
          ! ... Minimal radii for dry aerosol for the 3 log normal distribution
           double precision ,PARAMETER :: RCCN_MIN_3LN = 0.005D-4 ! [cm]
          double precision ,PARAMETER :: PI = 3.14159265D0
      ! ... Local
      
        NKR_local = NKR ! NOTE: we use 43 bins for aerosols
        ...
        CONCCCNIN = 0.0
          ...
          enddo
          FCCNR_bl = (FCCNR_bl + BL_ccncon2*dNbydlogD_norm2/sum(dNbydlogD_norm2))/col ! in [#/cm3/dln(r)]
          ...
          enddo
          FCCNR_ft = (FCCNR_ft + FT_ccncon2*dNbydlogD_norm2/sum(dNbydlogD_norm2))/col ! in [#/cm3/dln(r)]
         
! ---- JacobS: This is a particular setup of two modes for the ACPC test case
!              The user can use a single mode by zero-out the other mode
!              The aerosol size distribution in "FCCNR_bl" and "FCCNR_ft" are 
!              than distributed vertically for IC/BC in the main code. 
   ...
   ! +--------------------------------------------+
    SUBROUTINE HISCALE_aeroInput()

      implicit none

        ! Aerosol size for the SBM (RCCN) in [cm]
        ! Aerosl maximum and minimal values on the SBM grid (RCCN_MIN_3LN,RCCN_MAX)
        ! The output from this routine needs to be in units of [#/cm3/col]
        ! Follow the example in the "LogNormal_modes_Aerosol_ACPC" routine

        ! When the aerosol size distribution is created (say,"FCCNR_CK_HiScale")
        ! You need to: A. go to the main code to distributed it vertically.
        !              B. set it on the boundary and initial conditions. 

      return
    END SUBROUTINE HISCALE_aeroInput
   ! +--------------------------------------------+ 
    ...
    ! +----------------------------------------------------------------------------+
     ...
     USE module_mp_SBM_Nucleation,ONLY:JERNUCL01_KS, LogNormal_modes_Aerosol_ACPC,HISCALE_aeroInput
     ...
     INTEGER, PRIVATE,PARAMETER :: p_ff1i01=1,p_ff1i33=33,p_ff5i01=1,p_ff5i33=1, &
                                   p_ff6i01=1,p_ff6i33=1,p_ff8i01=34,p_ff8i33=66
                           
    PRIVATE
   
    PUBLIC FAST_SBM,FAST_HUCMINIT,falfluxhucm_z,ckern_z,j_w_melt,coal_bott_new,breakinit_ks,ecoalmass, &
    ...
    ! ... Holding Lookup tables and memory arrays for the FAST_SBM module
            ...
            double precision, ALLOCATABLE ::  cwll(:,:), &
                                                ...
                                                cwsg(:,:),cwss(:,:)
            double precision,ALLOCATABLE ::  FCCNR_ft(:),FCCNR_bl(:)
    ...
    double precision:: FR_LIM(NKR), FRH_LIM(NKR), lh_ce_1, lh_ce_2, lh_ce_3,  &
                          ...
                          del_icenucl_sum, del_ce_sum, del_ds_sum
   !---YZ2020-----------------------------------------------
   ...
   !--------------------------------------------------------
    ...
    !!!! NOTE: ZCGS AND OTHER VARIABLES ARE ALSO DIMENSIONED IN FALFLUXHUCM
       ...
       double precision:: w_stag, w_stag_my, qq_old,teten,es
       integer  print_int
    ...
    ! +---------------------------------------+
         if (itimestep == 1)then
          ...
          do k = kts, kte
             ...
             do KR = p_ff8i01, p_ff8i33
                KRR = KRR + 1
                write(dbg_msg, *) KRR, COL*chem_new(its,k,jts,KR) ! JS: Here aerosols are in [#/cm^3] 
    ...
    ! +--------------------------------------------+
       if (itimestep > 1 .and. dx > dx_bound)then
        ...
        DO j = j_start,j_end
           ...
           END DO
        END DO
        !print *, 'Kyle Killing the code', ICEPROCS
        !return !Kyle
        IF(ICEPROCS == 1)THEN
          KZ_Cloud_Base = 0
          DO j = jts,jte
            DO i = its,ite
                ...
                DO k = kts,kte
                    ...
                    if(SUP_WATER > 0.0D0 .and. w_stag_my > 0.1*1.0D2 &
                        .and. .NOT. K_found .and. K > 2 .and. zcgs(I,K,J) < 3.0*1.0D5) then
...
!--- JacobS: Commented out for this version -----------------------------------@
...
!---------------------------------------------------------@
   ...
   ! ... Graupel or Hail
    ...
    ! +----------------------------------+
      SUBROUTINE FAST_HUCMINIT(DT)
       ...
       double precision :: dlnr,ax,deg01,CONCCCNIN,CONTCCNIN
         ...
         character(len=256),parameter :: dir_43 = "SBM_input_33", dir_33 = "SBM_input_33"
    ...
    ! +-------------------------------------------------------+
       ...
       IF ( .TRUE. ) THEN
             print *,  'module_mp_FAST-SBM : Table-1 -- opening "BLKD_SDC.dat" on unit',hujisbm_unit1
...
#define DM_BCAST_MACRO_R16(A) CALL wrf_dm_bcast_bytes(A, size(A)*R16SIZE)
    ...
    ! +---------------------------------------------------+
        ...
        IF ( .TRUE. ) THEN
            print *, 'module_mp_FAST_SBM : Table-5 -- opening constants.asc on unit  ', hujisbm_unit1
            !CALL wrf_debug(150, errmess)
...
!---END CK COMMENT OUT----!
    ...
    if (.NOT. ALLOCATED(FCCN)) ALLOCATE(FCCN(NKR_aerosol))
       ...
       IF(ILogNormal_modes_Aerosol_ACPC == 1)THEN
          ...
          !CALL wrf_debug(000, errmess)
       ENDIF
       IF(IHISCALE_aeroInput == 1) THEN
         ! ... Initializing the FCCNR_HiScale from input
        CALL HISCALE_aeroInput()
       ENDIF
    ! +-------------------------------------------------------------+
