...
!---YZ2020Mar:change the aerosol size distribution from reading observed file and aerosol vertical distribution
!---
...
! +-----------------------------------------------------------------------------+
 ...
 public coll_xyy_lwf, coll_xyx_lwf, coll_xxx_lwf, &
        ...
        interpol_se, modkrn_KS, coll_breakup_KS, courant_bott_KS
...
! +-------------------------------------------------------------------------------+
    ...
    double precision :: tt_r, T_tab(7), SE_tab(7)
...
! +-----------------------------------------------------------------------------+
 ...
 public :: POLYSVP, JERRATE_KS, JERTIMESC_KS, JERSUPSAT_KS, &
 		       JERDFUN_KS, JERDFUN_NEW_KS, Relaxation_Time, CCN_regeneration, jernewf_ks
...
! +-----------------------------------------------------------------------------+
 ...
 private
 public JERNUCL01_KS, LogNormal_modes_Aerosol, WATER_NUCLEATION, ice_nucl, cloud_base_super, supmax_coeff
...
! +-----------------------------------------------------------------------------+
 SUBROUTINE JERNUCL01_KS(PSI1_r, PSI2_r, FCCNR_r, FCCNR_nucl_r, &
  ...
  double precision,intent(in) ::	 	   MWAERO, RO_SOLUTE
 	double precision,intent(inout) :: 	 PSI1_r(:),PSI2_r(:,:),FCCNR_r(:),FCCNR_nucl_r(:),lh_homo, lh_ice_nucl
 ...
 ! ... Locals
  ...
  double precision :: PSI1(NKR),PSI2(NKR,ICEMAX),FCCNR(NKR_aerosol),FCCNR_nucl(NKR_aerosol), ROR,XL(NKR),XI(NKR,ICEMAX),PP,COL, &
...
! +---------------------------------------+
 ...
 PUBLIC WARM_SBM,FAST_HUCMINIT,falfluxhucm_z,ckern_z,j_w_melt,coal_bott_new,breakinit_ks,ecoalmass, &
 ecoaldiam, ecoallowlist, ecoalochs, vtbeard, collenergy, onecond3, onecond2, onecond1, freez, &
 kernals_ks
...
! REAL,PARAMETER :: DX_BOUND = 1433
 ...
 ! ... Holding Lookup tables and memory arrays for the FAST_SBM module
         ...
         double precision,ALLOCATABLE ::  FCCNR_OBS(:),CCNR(:) !---YZ2020Mar:add for read-in aerosol SD
...
! distribution
 integer,parameter :: ILogNormal_modes_Aerosol = 1 ! read in a SD file from observation. Currently the file name for the observed SD is "CCN_size_33bin.dat", whcih is from the July 18 2017 ENA case.  
...
! added for diagCCN
   ...
   double precision :: gmax,tmax,qmax,divmax,rainmax,qnmax,inmax,knmax,hydro,difmax, tdif, tt_old, &
                         w_stag, w_stag_my, qq_old,teten,es
   ...
   ! ........................................................................
   double precision :: qna0(its:ite,kts:kte,jts:jte), fr_hom, w_stagm, CollEff_out, FACT, D_Mom_2, &
                         D_Mom_3, R_Mom_2, R_Mom_3
   double precision :: FACTZ_new(KMS:KME,NKR), TT_r, n_reg_ccn_bf, n_reg_ccn_af
! ### (KS) ............................................................................................
...
if (itimestep.eq.1)then
  if (iceprocs.eq.1) print *, " FAST SBM: ICE PROCESES ACTIVE "
  if (iceprocs.eq.0) print *, " FAST SBM: LIQUID PROCESES ONLY"
end if
...
!---YZ2020:Initialization at each timestep--------------@
      ...
      do k = kts,kte
        ...
        tempdiffl(I,K,J) = 0.0
        automass_tend(I,K,J) = 0.0
        autonum_tend(I,K,J) = 0.0
        nprc_tend(I,K,J) = 0.0
        n_reg_ccn(I,K,J) = 0.0
     end do
     end do
...
!-------------------------------------------------------@
     ...
     do j = jts,jte
        do k = kts,kte
          ...
          ! no explicit Ice particles in warm-sbm
          FF2R(:,1) = 0.0
          FF2R(:,2) = 0.0
          FF2R(:,3) = 0.0
          FF3R(:) = 0.0
        ...
        icenucl_bf = 0.0; lh_ice_nucl = 0.0; del_cldnucl_sum = 0.0;
        del_icenucl_sum = 0.0; del_ce_sum= 0.0; del_ds_sum=0.0;
...
! +---------------------------------------------+
        CCN_reg = 0.0
        Del_CCNreg = 0.0
        th_old(i,k,j) = t_new(i,k,j)/pi_phy(i,k,j)
        qv_old(i,k,j) = qv(i,k,j)
        IF (T_OLD(I,K,J).GT.193.15)THEN
           ...
           IF (QQA.LE.0) print *, "WARNING: FAST SBM, QQA < 0"
                ...
                DO IKL=1,NCOND
                  ...
                  IF (DIFFU.NE.0)THEN
                    ...
                    IF (DIFFU == 1)THEN
                      ...
                      IF (DEL2NR.EQ.0) print *, "fatal error in module_mp_fast_sbm (DEL2NR.EQ.0) , model stop "
...
!---YZ2020 / JacobS -------------  @
#ifdef SBM_DIAG
                     totlbf_diffu = sum(3.0*col*xl(:)*xl(:)*ff1r(:)/rhocgs(i,k,j))
...
#ifdef SBM_DIAG
                    totlaf_diffu = sum(3.0*col*xl(:)*xl(:)*ff1r(:)/rhocgs(i,k,j))
...
#endif
!----------------------------------------------------------@
                    ! Update temperature/water-vapor at the end of ONECOND and store it for tendencies
                    th_old(i,k,j) = TT/pi_phy(i,k,j)
                    qv_old(i,k,j) = QQ
...
!---YZ2020:divided by dt for process rate-----------------@
                ...
                autonum_tend(i,k,j)   = autonum_tend(i,k,j)  + autonum_ch/rhocgs(I,K,J)*1000.0/dt    ! #/kg/s
                nprc_tend(i,k,j)      = nprc_tend(i,k,j)     + nrautonum/rhocgs(I,K,J)*1000.0/dt     ! #/kg/s            
    ...
    DO k = kts,kte
      DO i = its,ite
        ...
        DO KR = p_ff1i01,p_ff1i33
          ...
          ELSE
            R_Mom_3 = R_Mom_3 + (DROPRADII(KRR)**3.0)*chem_new(i,k,j,KR)*XL(KRR)
            R_Mom_2 = R_Mom_2 + (DROPRADII(KRR)**2.0)*chem_new(i,k,j,KR)*XL(KRR)
            QR(I,K,J) = QR(I,K,J) &
        ...
        !IF(QC(I,K,J) > 1.E-6 .and. D_Mom_2 > 0.0) EFFR(I,K,J)    = (D_Mom_3/D_Mom_2)*1.0e4 			  ! microns
        !IF(QR(I,K,J) > 1.E-5 .and. R_Mom_2 > 0.0) R_EFFR(I,K,J)  = (R_Mom_3/R_Mom_2)*1.0e4 			  ! microns
...
! ..........................................
    if ( PRESENT (diagflag) ) then
        ...
        ! ... Drops
            KRR=0
            do kr = p_ff1i01,p_ff1i33     ! JS: Here DSDs input to Polar_HUCM is in [g/g/dln(r)]
                KRR=KRR+1
...
!
! JacobS: TH_OLD and QV_OD were moved to just after ONECOND block
!
!   do j=jts,jte
!   do k=kts,kte
!   do i=its,ite
 ...
 ! +----------------------------------+
   SUBROUTINE FAST_HUCMINIT(DT)
    ...
    double precision :: dlnr,ax,deg01,CONCCCNIN,CONTCCNIN
     ...
     print *, " FAST SBM: INITIALIZING WRF_HUJISBM "
     print *, " FAST SBM: ****** WRF_HUJISBM ******* "
...
! 	  CALL wrf_dm_bcast_integer ( usetables , size ( usetables ) * IWORDSIZE )
#endif
  WRITE(errmess, '(A,I2)') 'FAST_SBM_INIT : succesfull reading Table-10'
 ...
 ! +-------------------------+
 if (.NOT. ALLOCATED(fccnorig)) ALLOCATE(fccnorig(NKR_aerosol))
 if (.NOT. ALLOCATED(fccnd)) ALLOCATE(fccnd(NKR_aerosol))
 if (.NOT. ALLOCATED(FCCNR_MAR)) ALLOCATE(FCCNR_MAR(NKR_aerosol))
 if (.NOT. ALLOCATED(FCCNR_CON)) ALLOCATE(FCCNR_CON(NKR_aerosol))
 if (.NOT. ALLOCATED(FCCNR_OBS)) ALLOCATE(FCCNR_OBS(NKR_aerosol)) !---YZ2020Mar 
 if (.NOT. ALLOCATED(CCNR)) ALLOCATE(CCNR(NKR_aerosol)) !---YZ2020Mar
...
!---YZ2020Mar:read aerosol size distribution from observation----@
        ELSE ! read an observed SD with a format of aerosol size (cm), dN (#cm-3) and dNdlogD for 33bins (Jinwe Fan)
            ...
            OPEN(UNIT=hujisbm_unit1,FILE="CCN_size_33bin_20170718.dat",FORM="FORMATTED",STATUS="OLD",ERR=2070)
            do KR=1,NKR
               READ(hujisbm_unit1,*) RCCN(KR),CCNR(KR),FCCNR_OBS(KR) !---aerosol size (cm), dN (# cm-3) and dNdlogD for 33bins
              ! FCCNR_OBS(KR)=CCNR(KR)/COL
               end do
            CLOSE(hujisbm_unit1)
 ...
 IF(DEL1N >= 0.0D0) KCOND=11
   ...
   IF(KCOND == 11) THEN
      ...
      DTT = DTNEWL
   	  ...
   	  !IF (DTT < 0.0) print *, "fatal error in ONECOND1-DEL1N>0:(DTT<0), model stop"
...
CCN_reg = CCN_reg + max((totccn_before - totccn_after),0.0)
 ...
 ! +----------------------------------------------------------------------------+
         SUBROUTINE ONECOND3 &
 						 ...
 						 & ,COL,DTCOND,ICEMAX,NKR &
 						 & ,ISYM1,ISYM2,ISYM3,ISYM4,ISYM5, &
 						 	Iin,Jin,Kin,W_in,DX_in, Itimestep,CCN_reg)
